 ...
 */
constexpr unsigned int MD5_SIZE = 4 * sizeof(unsigned int);   /* 16 */
constexpr unsigned int MD5_STRING_SIZE = 2 * MD5_SIZE + 1;      /* 33 */
 ...
 namespace md5 {
    ...
    class md5_t {
        public:
             ...
             */
            md5_t(const void* input, unsigned int input_length, void* signature_ = nullptr);
             ...
             * input_length - The length of the buffer.
             */
            void process(const void* input, unsigned int input_length);
             ...
             * signature_ - A 16 byte buffer that will contain the MD5 signature.
             */
            void get_sig(void* signature_) const;
             ...
             */
            void get_string(void* str_) const;

        private:
            /* internal functions */
            //void initialise();
            void process_block(const unsigned char*);
            void get_result(void*) const;
            ...
            unsigned int message_length[2]{0, 0};     /* length of data */
            unsigned int stored_size;                   /* length of stored bytes */
            unsigned char stored[BLOCK_SIZE * 2];		/* stored bytes */
     ...
     */
    extern void sig_to_string(const void* signature, char* str, int str_len);
