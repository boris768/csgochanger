...
namespace jm {
    ...
    class xor_string {
        ...
        static
        XORSTR_FORCEINLINE void crypt_256_single(const std::uint64_t* keys,
        ...
        {
            (crypt_256_single(keys + Idxs * 4, _storage + Idxs * 4), ...);
        }

        static
        XORSTR_FORCEINLINE void crypt_128_single(const std::uint64_t* keys,
                                                  std::uint64_t* storage) noexcept
        {
            _mm_store_si128(
        ...
        XORSTR_FORCEINLINE void _crypt_128(const std::uint64_t* keys,
                                           std::index_sequence<Idxs...>) noexcept
        {
            (crypt_128_single(keys + Idxs * 2, _storage + Idxs * 2), ...);
        }
