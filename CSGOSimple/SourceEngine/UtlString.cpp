...
namespace SourceEngine
{
    ...
    void CUtlBinaryBlock::SetLength(int nLength)
    {
        ...
        if(nLength > m_Memory.NumAllocated()) {
	        const int nOverFlow = nLength - m_Memory.NumAllocated();
    ...
    int CUtlString::Format(const char *pFormat, ...)
    {
        ...
        int len = vsnprintf(tmpBuf, 4096, pFormat, marker);
        va_end(marker);
    ...
    void CUtlString::StripTrailingSlash()
    {
	    ...
	    const int nLastChar = Length() - 1;
	    const char c = m_Storage[nLastChar];
        if(c == '\\' || c == '/') {
