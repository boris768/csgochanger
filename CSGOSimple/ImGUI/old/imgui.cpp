...
// Debug options
constexpr auto IMGUI_DEBUG_NAV_SCORING = 0;	// Display navigation scoring preview when hovering items. Display last moving direction matches when holding CTRL
constexpr auto IMGUI_DEBUG_NAV_RECTS = 0;	// Display the reference navigation rectangle for each window
...
void*    GImAllocatorUserData = nullptr;
...
ImGuiIO::ImGuiIO(): MouseDown{}, MouseWheel(0), MouseWheelH(0), KeyCtrl(false), KeyShift(false), KeyAlt(false),
                    KeySuper(false),
                    KeysDown{},
                    NavInputs{},
                    ...
                    WantSaveIniSettings(false), NavActive(false),
                    NavVisible(false),
                    Framerate(0),
                    ...
                    MetricsActiveAllocations(0), MouseClickedTime{},
                    MouseClicked{},
                    MouseDoubleClicked{},
                    MouseReleased{}, MouseDownOwned{},
...
void ImGui::RenderNavHighlight(const ImRect& bb, ImGuiID id, ImGuiNavHighlightFlags flags)
{
    ...
    if (flags & ImGuiNavHighlightFlags_TypeDefault)
    {
		...
		const auto half_thickness = ImVec2(THICKNESS * 0.5f, THICKNESS * 0.5f);
        window->DrawList->AddRect(display_rect.Min + half_thickness, display_rect.Max - half_thickness, GetColorU32(ImGuiCol_NavHighlight), rounding, ImDrawCornerFlags_All, THICKNESS);
        if (!fully_visible)
...
void ImGui::UpdateMouseWheel()
{
    ...
    {
        const ImVec2 max_step = (window->ContentsRegionRect.GetSize() + window->WindowPadding * 2.0f) * 0.67f;

        // Vertical Mouse Wheel Scrolling (hold Shift to scroll horizontally)
        if (g.IO.MouseWheel != 0.0f && !g.IO.KeyShift)
        {
	        const float scroll_step = ImFloor(ImMin(5 * window->CalcFontSize(), max_step.y));
            SetWindowScrollY(window, window->Scroll.y - g.IO.MouseWheel * scroll_step);
        }
        else if (g.IO.MouseWheel != 0.0f && g.IO.KeyShift)
        {
	        const float scroll_step = ImFloor(ImMin(2 * window->CalcFontSize(), max_step.x));
...
void ImGui::Render()
{
    ...
    for (auto& n : windows_to_render_front_most)
	    if (n && IsWindowActiveAndVisible(n)) // NavWindowingTarget is always temporarily displayed as the front-most window
...
bool ImGui::IsItemFocused()
{
    ...
    return !(g.NavId == 0 || g.NavDisableHighlight || g.NavId != window->DC.LastItemId);
}
...
static ImVec2 CalcSizeAutoFit(ImGuiWindow* window, const ImVec2& size_contents)
{
    ...
    else
    {
		...
		const ImVec2 size_auto_fit_after_constraint = CalcSizeAfterConstraint(window, size_auto_fit);
...
{
	const ImVec2 size_contents = CalcContentSize(window);
...
static void CalcResizePosSizeFromAnyCorner(ImGuiWindow* window, const ImVec2& corner_target, const ImVec2& corner_norm, ImVec2* out_pos, ImVec2* out_size)
{
	...
	const ImVec2 size_constrained = CalcSizeAfterConstraint(window, size_expected);
    *out_pos = pos_min;
...
void ImGui::RenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
    ...
    else
    {
        ...
        if (flags & ImGuiWindowFlags_MenuBar)
        {
			...
			ImVec2 vec_window_border_size(window_border_size, 0);
            window->DrawList->AddRectFilled(menu_bar_rect.Min + vec_window_border_size, menu_bar_rect.Max - vec_window_border_size, GetColorU32(ImGuiCol_MenuBarBg), (flags & ImGuiWindowFlags_NoTitleBar) ? window_rounding : 0.0f, ImDrawCornerFlags_Top);
...
void ImGui::RenderWindowTitleBarContents(ImGuiWindow* window, const ImRect& title_bar_rect, const char* name, bool* p_open)
{
    ...
    if (flags & ImGuiWindowFlags_UnsavedDocument)
    {
		...
		const ImVec2 off = ImVec2(0.0f, (float)(int)(-g.FontSize * 0.25f));
...
void ImGui::End()
{
    ...
    if (window->DC.CurrentColumns != nullptr)
        EndColumns();
...
void ImGui::OpenPopupEx(ImGuiID id)
{
    ...
    else
    {
		...
		auto& open_popup_stack_top = g.OpenPopupStack[current_stack_size];
    	if (open_popup_stack_top.PopupId == id && open_popup_stack_top.OpenFrameCount == g.FrameCount - 1)
        {
			open_popup_stack_top.OpenFrameCount = popup_ref.OpenFrameCount;
        }
        else
        {
            // Close child popups if any, then flag popup for open/reopen
            g.OpenPopupStack.resize(current_stack_size + 1);
			open_popup_stack_top = popup_ref;
        }
