...
namespace ImGui
{
    ...
    // Styles
	IMGUI_API void          StyleColorsBlack(ImGuiStyle* style = nullptr);
...
constexpr auto IMGUI_PAYLOAD_TYPE_COLOR_3F = "_COL3F";    // float[3]: Standard type for colors, without alpha. User code may use this type.
constexpr auto IMGUI_PAYLOAD_TYPE_COLOR_4F = "_COL4F";    // float[4]: Standard type for colors. User code may use this type.
...
struct ImVector
{
    ...
    [[nodiscard]] int          size_in_bytes() const               { return Size * (int)sizeof(T); }
    ...
    [[nodiscard]] const T&     front() const                       { IM_ASSERT(Size > 0); return Data[0]; }
    T&           back()                              { IM_ASSERT(Size > 0); return Data[Size - 1]; }
    [[nodiscard]] const T&     back() const                        { IM_ASSERT(Size > 0); return Data[Size - 1]; }
    void         swap(ImVector<T>& rhs) noexcept
    {
	    const int rhs_size = rhs.Size;
    	rhs.Size = Size;
    	Size = rhs_size;
	    const int rhs_cap = rhs.Capacity;
...
struct ImGuiIO
{
    ...
    IMGUI_API void  AddInputCharactersUTF8(const char* utf8_chars);    // Queue new characters input from an UTF-8 string
...
namespace ImGui
{
    ...
    static void SetNextTreeNodeOpen(bool open, ImGuiCond cond = 0)   { SetNextItemOpen(open, cond); }
    // OBSOLETED in 1.70 (from May 2019)
    static float GetContentRegionAvailWidth()          { return GetContentRegionAvail().x; }
    // OBSOLETED in 1.69 (from Mar 2019)
    static ImDrawList* GetOverlayDrawList()            { return GetForegroundDrawList(); }
    // OBSOLETED in 1.66 (from Sep 2018)
    static void  SetScrollHere(float center_ratio=0.5f){ SetScrollHereY(center_ratio); }
    // OBSOLETED in 1.63 (between Aug 2018 and Sept 2018)
    static bool  IsItemDeactivatedAfterChange()        { return IsItemDeactivatedAfterEdit(); }
    ...
    static ImVec2 CalcItemRectClosestPoint(const ImVec2& pos, bool on_edge = false, float outward = 0.f) { IM_UNUSED(on_edge); IM_UNUSED(outward); IM_ASSERT(0); return pos; }
    // OBSOLETED in 1.53 (between Oct 2017 and Dec 2017)
    static void  ShowTestWindow()                      { return ShowDemoWindow(); }
    static bool  IsRootWindowFocused()                 { return IsWindowFocused(ImGuiFocusedFlags_RootWindow); }
    static bool  IsRootWindowOrAnyChildFocused()       { return IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows); }
    static void  SetNextWindowContentWidth(float w)    { SetNextWindowContentSize(ImVec2(w, 0.0f)); }
    static float GetItemsLineHeightWithSpacing()       { return GetFrameHeightWithSpacing(); }
    ...
    static bool  IsRootWindowOrAnyChildHovered()       { return IsWindowHovered(ImGuiHoveredFlags_RootAndChildWindows); }
    static void  AlignFirstTextHeightToWidgets()       { AlignTextToFramePadding(); }
    static void  SetNextWindowPosCenter(ImGuiCond c=0) { ImGuiIO& io = GetIO(); SetNextWindowPos(ImVec2(io.DisplaySize.x * 0.5f, io.DisplaySize.y * 0.5f), c, ImVec2(0.5f, 0.5f)); }
    // OBSOLETED in 1.51 (between Jun 2017 and Aug 2017)
    static bool  IsItemHoveredRect()                   { return IsItemHovered(ImGuiHoveredFlags_RectOnly); }
    static bool  IsPosHoveringAnyWindow(const ImVec2&) { IM_ASSERT(0); return false; } // This was misleading and partly broken. You probably want to use the ImGui::GetIO().WantCaptureMouse flag instead.
    static bool  IsMouseHoveringAnyWindow()            { return IsAnyWindowHovered(); }
    static bool  IsMouseHoveringWindow()               { return IsWindowHovered(ImGuiHoveredFlags_AllowWhenBlockedByPopup | ImGuiHoveredFlags_AllowWhenBlockedByActiveItem); }
}
...
struct ImGuiTextFilter
{
    ...
    struct TextRange
    {
        ...
        [[nodiscard]] const char*     begin() const   { return b; }
        [[nodiscard]] const char*     end () const    { return e; }
...
struct ImDrawCmd
{
    ...
    ImDrawCmd() { ElemCount = 0; ClipRect.x = ClipRect.y = ClipRect.z = ClipRect.w = 0.0f; TextureId = (ImTextureID)nullptr; VtxOffset = IdxOffset = 0;  UserCallback = nullptr; UserCallbackData = nullptr; }
};
...
struct ImDrawList
{
    ...
    [[nodiscard]] ImVec2   GetClipRectMin() const { const ImVec4& cr = _ClipRectStack.back(); return ImVec2(cr.x, cr.y); }
    [[nodiscard]] ImVec2   GetClipRectMax() const { const ImVec4& cr = _ClipRectStack.back(); return ImVec2(cr.z, cr.w); }
...
struct ImFontGlyphRangesBuilder
{
    ...
    void     Clear()             { int size_in_bytes = 0x10000 / 8; UsedChars.resize(size_in_bytes / (int)sizeof(ImU32)); memset(UsedChars.Data, 0, (size_t)size_in_bytes); }
    [[nodiscard]] bool     GetBit(int n) const { int off = (n >> 5); ImU32 mask = 1u << (n & 31); return (UsedChars[off] & mask) != 0; }  // Get bit n in the array
    void     SetBit(int n)       { int off = (n >> 5); ImU32 mask = 1u << (n & 31); UsedChars[off] |= mask; }               // Set bit n in the array
...
struct ImFontAtlas
{
    ...
    [[nodiscard]] const CustomRect*   GetCustomRectByIndex(int index) const { if (index < 0) return nullptr; return &CustomRects[index]; }

    // [Internal]
    IMGUI_API void      CalcCustomRectUV(const CustomRect* rect, ImVec2* out_uv_min, ImVec2* out_uv_max) const;
...
struct ImFont
{
    ...
    [[nodiscard]] bool                        IsLoaded() const                    { return ContainerAtlas != nullptr; }
