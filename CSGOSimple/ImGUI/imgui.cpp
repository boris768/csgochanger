...
void ImGui::RenderNavHighlight(const ImRect& bb, ImGuiID id, ImGuiNavHighlightFlags flags)
{
    ...
    if (flags & ImGuiNavHighlightFlags_TypeDefault)
    {
		...
		constexpr float DISTANCE = 3.0f + THICKNESS * 0.5f;
        display_rect.Expand(ImVec2(DISTANCE,DISTANCE));
        bool fully_visible = window->ClipRect.Contains(display_rect);
        if (!fully_visible)
            window->DrawList->PushClipRect(display_rect.Min, display_rect.Max);
		const auto vec2 = ImVec2(THICKNESS * 0.5f, THICKNESS * 0.5f);
        window->DrawList->AddRect(
			display_rect.Min + vec2,
			display_rect.Max - vec2,
...
void ImGui::RenderWindowDecorations(ImGuiWindow* window, const ImRect& title_bar_rect, bool title_bar_is_highlight, int resize_grip_count, const ImU32 resize_grip_col[4], float resize_grip_draw_size)
{
    ...
    else
    {
        ...
        if (flags & ImGuiWindowFlags_MenuBar)
        {
			...
			const auto vec2 = ImVec2(window_border_size, 0);
        	window->DrawList->AddRectFilled(
				menu_bar_rect.Min + vec2,
				menu_bar_rect.Max - vec2,
...
void ImGui::OpenPopupEx(ImGuiID id)
{
    ...
    else
    {
		...
		auto open_popup_current_stack = &g.OpenPopupStack[current_stack_size];
        if (open_popup_current_stack->PopupId == id && open_popup_current_stack->OpenFrameCount == g.FrameCount - 1)
        {
			open_popup_current_stack->OpenFrameCount = popup_ref.OpenFrameCount;
        }
        else
        {
...
ImVec2 ImGui::FindBestWindowPosForPopup(ImGuiWindow* window)
{
    ...
    if (window->Flags & ImGuiWindowFlags_Tooltip)
    {
        ...
        float sc1 = g.Style.MouseCursorScale;
        ImVec2 ref_pos = NavCalcPreferredRefPos();
        ImRect r_avoid;
        if (!g.NavDisableHighlight && g.NavDisableMouseHover && !(g.IO.ConfigFlags & ImGuiConfigFlags_NavEnableSetMousePos))
            r_avoid = ImRect(ref_pos.x - 16, ref_pos.y - 8, ref_pos.x + 16, ref_pos.y + 8);
        else
            r_avoid = ImRect(ref_pos.x - 16, ref_pos.y - 8, ref_pos.x + 24 * sc1, ref_pos.y + 24 * sc1); // FIXME: Hard-coded based on mouse cursor shape expectation. Exact dimension not very important.
