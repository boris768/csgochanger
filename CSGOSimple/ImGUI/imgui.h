...
{
    float     x, y;
    constexpr ImVec2() noexcept : x(0.f), y(0.f)  {}
	constexpr ImVec2(const float _x, const float _y) noexcept : x(_x), y(_y) {}
...
{
    float     x, y, z, w;
    constexpr ImVec4() noexcept : x(0.f),y(0.f),z(0.f),w(0.f) {}
	constexpr ImVec4(float _x, float _y, float _z, float _w) noexcept : x(_x), y(_y), z(_z), w(_w) {}
...
struct ImVector
{
    ...
    void         swap(ImVector<T>& rhs)              { int rhs_size = rhs.Size; rhs.Size = Size; Size = rhs_size; int rhs_cap = rhs.Capacity; rhs.Capacity = Capacity; Capacity = rhs_cap; T* rhs_data = rhs.Data; rhs.Data = Data; Data = rhs_data; }
...
struct ImGuiTextFilter
{
    ...
    struct ImGuiTextRange
    {
        ...
        constexpr ImGuiTextRange() noexcept : b(nullptr), e(nullptr){ }
		constexpr ImGuiTextRange(const char* _b, const char* _e) noexcept : b(_b), e(_e) { }
...
struct ImColor
{
    ...
    constexpr ImColor() {}
    constexpr ImColor(int r, int g, int b, int a = 255) :Value((float)r*sc, (float)g* sc, (float)b* sc, (float)a* sc)
    {
    }
    ImColor(ImU32 rgba)                                             { Value.x = (float)((rgba>>IM_COL32_R_SHIFT)&0xFF) * sc; Value.y = (float)((rgba>>IM_COL32_G_SHIFT)&0xFF) * sc; Value.z = (float)((rgba>>IM_COL32_B_SHIFT)&0xFF) * sc; Value.w = (float)((rgba>>IM_COL32_A_SHIFT)&0xFF) * sc; }
    ImColor(float r, float g, float b, float a = 1.0f)              { Value.x = r; Value.y = g; Value.z = b; Value.w = a; }
    constexpr ImColor(const ImVec4& col) noexcept:Value(col)                           { /*Value = col;*/ }
    operator ImU32() const                                   { return ImGui::ColorConvertFloat4ToU32(Value); }
    constexpr operator ImVec4()noexcept                                  { return Value; }
...
struct ImFontGlyphRangesBuilder
{
    ...
    bool     GetBit(int n) const { int off = (n >> 5); ImU32 mask = 1u << (n & 31); return (UsedChars[off] & mask) != 0; }  // Get bit n in the array
